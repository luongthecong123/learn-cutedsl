...
| Architecture | Instruction | Supported Shapes | Fragment Location | Notes |
| :--- | :--- | :--- | :--- | :--- |
| **Ampere** (SM 80+) | `mma.sync` | $16 \times 8 \times 16$<br>$16 \times 8 \times 8$<br>$8 \times 8 \times 4$ | fragA: RF<br>fragB: RF<br>fragC: RF | $K=16$ is the standard for dense FP16; <br> $K=8$ and $K=4$ are legacy. |
| **Hopper** (SM 90+) | `wgmma.mma_async` | $64 \times N \times 16$<br> | fragA: SMEM or RF<br>fragB: SMEM<br>fragC: RF | $N$ must be a multiple of 8 up to 256 |
| **Blackwell** (SM 100+) | `tcgen05.mma` | $64 \times N \times 16$<br>$128 \times N \times 16$<br>$256 \times N \times 16$ (2 CTA) | fragA: SMEM or TMEM<br>fragB: SMEM<br>fragC: TMEM | $N$ must be a multiple of 8 up to 256. <br> $M=256$ requires 2 CTA mma |
...
Note: barrier_id = 0 is reserved for cute.arch.sync_threads(), any attempts to use id 0 for barrier will cause run time error. If you were to map barrier_id to warp or warp_group indices, make sure to +1 to avoid assigning id 0.
...
To close the remaining gap to cuBLAS (~720 TFLOPS on H100, ~1500 TFLOPS on B200), the next step is persistent kernels — a technique that keeps the CTA alive across multiple output tiles, overlapping the epilogue of the current tile (casting to FP16 and storing to global memory in this GEMM case) with the computation of the next tile. Apart from persistent kernel and overlapping epilogue:
- On Hopper, kernel c2 would gain from TMA multicast and CTA clusters.
