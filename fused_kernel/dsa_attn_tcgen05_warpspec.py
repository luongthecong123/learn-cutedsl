...
Warp specialization + persistent kernel
UMMA workers: use tensor core on full split.
SGEMM workers: use SIMT on partial/tail split.

Kernel flow
1. Work assignment, swizzle (rotation) split, which split is full and partial
2. Each worker loop through the buffer to get its split
UMMA and SGEMM worker run concurrently and independant of each other.
"""
import cutlass
...
class Dsa():
    ...
    def compute_kernel(
        ...
        # 128-bit cpasync for KPE: 8 threads × 8 bf16 = 64 = HEAD_DIM_KPE per copy
        atom_cpa_pe   = cute.make_copy_atom(cpasync.CopyG2SOp(), cutlass.BFloat16, num_bits_per_copy=128)
        thr_layout_pe = cute.make_layout(((8, 1),), stride=((1, 8),))
        val_layout_pe = cute.make_layout(((8, 1),), stride=((1, 0),))
        tiled_copy_pe = cute.make_tiled_copy_tv(atom_cpa_pe, thr_layout_pe, val_layout_pe)
        lane_copy_pe  = tiled_copy_pe.get_slice(lane_idx % 8)
