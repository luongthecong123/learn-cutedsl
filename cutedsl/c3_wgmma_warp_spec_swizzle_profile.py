...
class Gemm_TC:
    ...
    def kernel(
        ...
        if warp_idx == 0:
            if cutlass.const_expr(use_measure):
                if tidx == 0:
                    range_start(probe, tma_row, tma_cnt, sm, TAGS["tma_store"])
...
def main():
    ...
    max_entries_per_role = (K // bK) + 2  # k-tiles + frag_to_smem + tma_store
    probe_cols = PROBE_HEADER + max_entries_per_role * PROBE_ENTRY
    probe = torch.zeros((num_blocks * NUM_PROBE_ROLES, probe_cols),
    ...
    # Correctness check with small problem
    M_s, N_s, K_s = 128, 128, 128
    A_s = torch.randn((M_s, K_s), device="cuda", dtype=torch.float16)
    B_s = torch.randn((N_s, K_s), device="cuda", dtype=torch.float16)
    C_s = torch.empty((M_s, N_s), device="cuda", dtype=torch.float16)
    num_blocks_s = (N_s // bN) * (M_s // bM)
    probe_s = torch.zeros((num_blocks_s * NUM_PROBE_ROLES, probe_cols),
                          device="cuda", dtype=torch.int64)
    A_s_, B_s_, C_s_, probe_s_ = [from_dlpack(t, assumed_align=16)
                                   for t in (A_s, B_s, C_s, probe_s)]
    compiled_small = cute.compile(gemm, A_s_, B_s_, C_s_, probe_s_, True)
    compiled_small(A_s_, B_s_, C_s_, probe_s_)
    assert torch.allclose(C_s, torch.matmul(A_s, B_s.T), atol=1e-1, rtol=1e-1), "CORRECTNESS FAILED"
